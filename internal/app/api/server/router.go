...
import (
	"akcom/internal/app/api/middleware/auth"
	"akcom/internal/app/api/middleware/jwt"
	"akcom/internal/app/apiclient"
	"akcom/internal/app/constants"
	exchangeController "akcom/internal/app/controller/exchange"
	"akcom/internal/app/controller/healthcheck"
	"akcom/internal/app/controller/oauth"
	userController "akcom/internal/app/controller/user"

	"akcom/internal/app/db"
	exchangeDBClient "akcom/internal/app/db/repository/exchange"
	userDBClient "akcom/internal/app/db/repository/user"
...
func NewRouter(ctx context.Context, dbConnection *db.DBService) *gin.Engine {
	...
	var (
		...
		exchangeDBClient = exchangeDBClient.NewExchangeRepository(dbConnection)
	)
	...
	var (
		...
		exchangeClient = apiclient.NewExchange(api)
	)

	// Controller
	var (
		userController = userController.NewUserController(userDBClient)

		oAuthController       = oauth.NewOAuthController(userDBClient, jwt)
		healthCheckController = healthcheck.NewHealthCheckController()
		exchangeController    = exchangeController.NewExchangeController(exchangeClient, exchangeDBClient)
	)

	go exchangeController.GetPreviousTenDaysExchangeRates(ctx)

	v1 := router.Group("/akcom/v1")
	{
		...
		{
			authenticated.Use(auth.Authentication(jwt, []constants.ROLE{constants.ADMIN, constants.USER})) // pass allowed roles for the APIs

			authenticated.GET(PROFILE, userController.GetProfile)
			authenticated.PATCH(PROFILE, userController.UpdateProfile)

			authenticated.GET(EXCHANGE, exchangeController.GetExchangeRates)
		}

		adminAuthenticated := v1.Group("/admin")
		{
			adminAuthenticated.Use(auth.Authentication(jwt, []constants.ROLE{constants.ADMIN})) // pass allowed roles for the APIs

			adminAuthenticated.POST(SIGN_UP, oAuthController.SignUp)

			adminAuthenticated.GET(USERS, oAuthController.GetAllUser)
			adminAuthenticated.PATCH(USERS, oAuthController.DisableUser)
		}
	}
