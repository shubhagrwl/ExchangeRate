...
type IJwtService interface {
	...
	VerifyToken(ctx context.Context, tokenString string, role []constants.ROLE) (*users_DBModels.Users, bool)
}
...
func (j *JwtService) VerifyToken(ctx context.Context, tokenString string, role []constants.ROLE) (*users_DBModels.Users, bool) {
	log := logger.Logger(ctx)
	...
	if ok && token.Valid {
		...
		u, err := j.UserDBClient.GetUser(ctx, fmt.Sprintf("%s='%s'",
			users_DBModels.COLUMN_EMAIL, user.Email))
		if err != nil || !u.Status || !util.IsRoleContain(role, constants.ROLE(u.Role)) {
			return nil, false
		}
